...
// Locates the first value char for `key` whose type matches `allowed_first`.
// Iterates over all marker occurrences so a key name appearing inside a
...
// unescaping the common sequences. Advances `at` past the closing quote.
...
// Extracts the content between the `[` at `at` and its matching `]`,

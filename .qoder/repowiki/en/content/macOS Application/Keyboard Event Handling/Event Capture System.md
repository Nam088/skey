...
## Update Summary
**Changes Made**
- Updated EventTapManager section to reflect callback-based detection replacing watchdog timer approach
- Added new automatic quit behavior when permissions are revoked
- Updated architecture diagrams to show instant detection mechanism
- Enhanced troubleshooting guide with new permission loss handling
...
**Updated** The system now uses callback-based detection instead of watchdog timers for instant event pass-through when permissions are revoked, automatically quitting the application to prevent UI freezing issues.
...
## Core Components
- EventTapManager: Creates and manages a CGEventTap, sets up a dedicated CFRunLoop thread, handles tap enable/disable, and recovers from tap timeouts using callback-based detection.
...
- Events are delivered to EventTapManager.handleEvent(), which first recovers from tap-disabled states using callback-based detection, then delegates to TypingPipeline.process().
...
- When permissions are revoked, the system automatically quits using exit(0) to prevent UI freezing, requiring user restart after re-granting permissions.
...
ETM->>ETM : handleEvent() detect tapDisabledByTimeout/UserInput
ETM->>ETM : Pass through ALL events immediately
ETM->>ETM : exit(0) - automatic quit
...
- **Updated Error Recovery**: Uses callback-based detection instead of watchdog timers. In handleEvent(), detects .tapDisabledByTimeout or .tapDisabledByUserInput and immediately passes through all events before automatically quitting the app using exit(0) to prevent UI freezing.
...
RunLoop --> Callback{"Callback receives event"}
Callback --> CheckType{"type == tapDisabled?"}
CheckType -- Yes --> PassThrough["Pass through ALL events FIRST"]
PassThrough --> QuitApp["exit(0) - automatic quit"]
CheckType -- No --> Process["Normal event processing"]
...
- Uses caching to avoid expensive system calls on hot paths with 5-second cache validity.
...
- **Updated** Callback-based detection eliminates watchdog timer overhead, providing instant response to permission changes without polling.
...
- **Updated Permission Loss Handling**: When permissions are revoked, the system automatically quits using exit(0) to prevent UI freezing. Users must restart the application after re-granting permissions.
- **Updated Instant Detection**: The callback-based detection system provides immediate response to permission changes without watchdog timer delays.
...
## Conclusion
The system implements a robust, low-latency event capture pipeline using CoreGraphics EventTap with a dedicated CFRunLoop thread. It employs a pragmatic fallback strategy for tap creation, precise event masks for keyboard and mouse events, and resilient error recovery for tap timeouts using callback-based detection. **Updated** The system now automatically quits when permissions are revoked to prevent UI freezing, eliminating the need for watchdog timers and providing instant response to permission changes. Permissions are checked and guided to users, while synthetic event injection maintains compatibility across applications. The design balances performance and reliability, making it suitable for real-time text composition and global hotkey interception.